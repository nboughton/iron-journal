  ...
  <q-card class="my-card asset-card column">
    ...
    <q-card-section class="col justify-evenly">
      ...
      <div v-for="(item, index) in data.items" :key="index" class="row items-start q-pb-sm">
        ...
        <div class="col-1" /><q-input class="col-11 self-end" v-if="item.input" :label="item.input.label" :disable="browse" dense debounce="750" v-model="data.items[index].input.text" />
      </div>
    </q-card-section>
